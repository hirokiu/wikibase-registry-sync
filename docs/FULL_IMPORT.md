...
WikibaseSyncで重視していた「投入先にデータが存在するか」「修正点があるか」を確認する方針を継承する。SPARQLを問い合わせること自体を要件とはせず、投入先の現在データを確認することを要件とする。

1. 対象の一覧を取得し、レコードキーの存在・一意性を確認する。
2. 該当するItemをAction APIから読み、ラベル・対象文・出典・限定子を比較する。
3. 存在しなければ作成、一致していれば書き込まない。
4. 差分がある場合はモードに従う。initial-loadは報告して停止、将来のupsertは管理対象の変更点だけを更新する。upsertでは読み取り後の競合もrevisionで検出する方針とする。
5. 作成・更新後の読み戻しと、バッチ終了後の全件verifyを行う。

存在確認を効率化するため、一覧はページ単位・Itemは最大50件ずつ取得し、索引をSQLiteに保存する。該当Itemの比較時には現在値を取得する。入力の各行ごとに全体検索をやり直す方式ではない。ローカルの記録や過去の検索結果だけを根拠に差分なしと判定しない。対象キーの並行作成に関する単一writerの制約は引き続き適用する。

同一入力・同一ジョブの途中再開ではdoneを省略する。これは処理再開のための記録であり、現在値の再確認とは区別する。全件の最新状態を確認するにはverifyを使い、新Snapshotには新しいジョブを作る。

|モード|存在・差分確認|差分の扱い|
|---|---|---|
|initial-load（実装済み）|実施|報告して停止|
|upsert（今後実装）|必須|管理対象の変更点だけ更新|
|確認を省略する初期投入|未実装・標準にしない|空の対象・単独writerを保証する必要性が出た場合に別途検討|

したがって、現時点で初期投入を指定しても確認は省略されない。修正点の検出と、修正を自動適用する機能は分けて扱う。
